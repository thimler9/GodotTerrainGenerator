...
float simplex_noise(SimplexNoiseParams params, uvec3 id, vec3 chunk_offset, uint lod) {
    ...
    for (uint i = 0; i < num_octaves; i++)
    {
        ...
        vec3 sample_point = (chunk_offset.xyz + seed_value * vec3(1.0) + vec3(id * lod)) / scale * frequency;
